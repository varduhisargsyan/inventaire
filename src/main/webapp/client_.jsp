    ...
    <div class="row content">
        ...
        <div class="col-md-10 content_body">
            <table class="pagination">
                <tr>
                    <th class="narrow">#</td>
                    <th><s:text name="text.first_name"/></th>
                    <th><s:text name="text.last_name"/></th>
                    <th><s:text name="text.tel_fix"/></th>
                    <th><s:text name="text.tel_mob"/></th>
                    <th><s:text name="text.mail"/></th>
                    <th><s:text name="text.address"/></th>
                    <th ><a href="<s:url action="client_add"/>" class="btn btn-primary btn-sm"> <s:text name="text.add"/></a>
                    </th>

                </tr>
                <s:if test="clientList!=null">
                <s:iterator value="clientList" status="status" var="client">
                    <tr>
                        <td class="narrow"><s:property value="#status.count"/></td>
                        <td><s:property value="#client.fname"/></td>
                        <td><s:property value="#client.lname"/></td>
                        <td><s:property value="#client.nfix"/></td>
                        <td><s:property value="#client.nmob"/></td>
                        <td><s:property value="#client.email"/></td>
                        <td><s:property value="#client.address"/></td>
                        <td class="controls">
                            <s:url action="client_edit" var="edit_url">
                                <s:param name="id" value="#client.id"/>
                            </s:url>
                            <s:url action="client_delete" var="delete_url">
                                <s:param name="id" value="#client.id"/>
                            </s:url>
